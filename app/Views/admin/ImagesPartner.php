        ...
        <div class="col-lg-12">
          <div class="card">
            <div class="card-body">
              ...
              <form action="<?= site_url('admin/ImagesPartner/save') ?>" method="POST" enctype="multipart/form-data">
                <?= csrf_field() ?>
                ...
                <div class="row mb-3">
                  <label for="name" class="col-sm-2 col-form-label">Image Name</label>
                  <div class="col-sm-10">
                    <input type="text" id="nama" name="nama" class="form-control" value="<?= isset($image) ? $image['nama'] : '' ?>" required>
                  </div>
                </div>
                ...
                <div class="row mb-3">
                  ...
                  <div class="col-sm-10">
                    <input type="text" id="deskripsi" name="deskripsi" class="form-control" value="<?= isset($image) ? $image['deskripsi'] : '' ?>" required>
                  </div>
                </div>
                ...
                <?php if (isset($image)): ?>
                  <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                      <img src="<?= base_url('images/' . $image['image']) ?>" style="max-width: 200px; display: block;">
                    </div>
                  </div>
                <?php endif; ?>
                ...
                <div class="row mb-3">
                  <div class="col-sm-10 offset-sm-2">
                    ...
                    <a href="<?= site_url('admin/ImagesPartner') ?>" class="btn btn-secondary">Cancel</a>
                  </div>
                </div>
              </form>
          ...
          <div class="card mt-4">
            <div class="card-body">
              <h5 class="card-title">Image Partner dan Media</h5>
              ...
              <table class="table datatable">
                <thead>
                  <tr>
                    <th>No</th>
                    <th>Image Name</th>
                    <th>Description</th> <!-- Kolom untuk deskripsi -->
                    <th>Image Preview</th>
                    <th>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  <?php if (!empty($imagespar)): ?>
                    <?php foreach ($imagespar as $index => $image): ?>
                      <tr>
                        <td><?= $index + 1 ?></td>
                        <td><?= esc($image['nama']) ?></td>
                        <td><?= esc($image['deskripsi']) ?></td> <!-- Tampilkan deskripsi -->
                        <td><img src="<?= base_url('images/' . $image['image']) ?>" style="max-width: 100px;"></td>
                        <td>
                          <a href="<?= site_url('admin/ImagesPartner/edit/' . $image['id']) ?>" class="btn btn-warning btn-sm">Edit</a>
                          <a href="<?= site_url('admin/ImagesPartner/delete/' . $image['id']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this image?');">Delete</a>
                        </td>
                      </tr>
                    <?php endforeach; ?>
                  <?php else: ?>
  ...
  <script>
    ...
    $(document).ready(function() {
      $('.status-toggle').click(function() {
        ...
        var newStatus = currentStatus === 'checked' ? 'unchecked' : 'checked';
        ...
        $.ajax({
          ...
          success: function(response) {
            if (response.success) {
              span.data('status', newStatus);
              span.text(newStatus === 'checked' ? '✔️' : '❌');
              span.css('color', newStatus === 'checked' ? 'green' : 'red');
            } else {
